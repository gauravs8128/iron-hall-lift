...
function Index() {
  ...
  return (
    <main className="premium-grain overflow-hidden bg-background text-foreground">
      {introVisible && <div className="intro-screen" aria-hidden="true"><div className="intro-word"><span>IRON</span><span className="text-primary">VAULT</span></div><div className="intro-line"/><p>Strength / Discipline / Progress</p></div>}
      <header className="fixed inset-x-0 top-0 z-50 border-b border-foreground/10 bg-background/70 backdrop-blur-xl">
        <div className="mx-auto flex h-20 max-w-7xl items-center justify-between px-5 lg:px-8">
          <a href="#top" className="flex items-center gap-3" aria-label="Ironvault home">
            <span className="grid size-9 place-items-center border border-foreground/20 bg-foreground text-background"><Dumbbell size={19} /></span>
            <span><span className="block font-display text-xl uppercase leading-none">Iron<span className="text-primary">vault</span></span><span className="mt-1 hidden text-[7px] font-semibold uppercase tracking-[0.28em] text-muted-foreground lg:block">Private strength club</span></span>
          </a>
      ...
      <section id="top" className="relative flex min-h-[94svh] items-end pt-24">
        ...
        <div className="hero-shade absolute inset-0" /><div className="hero-frame absolute inset-4 top-24 hidden border border-foreground/10 md:block" />
        <div className="relative mx-auto grid w-full max-w-7xl gap-12 px-5 pb-16 lg:grid-cols-[1fr_300px] lg:px-8 lg:pb-20">
          <div className="animate-lift-in max-w-4xl">
            <div className="mb-6 flex items-center gap-3 text-xs font-bold uppercase tracking-[0.2em] text-muted-foreground"><span className="h-px w-10 bg-primary" />Est. 2018 &nbsp;·&nbsp; Built for the work</div>
          ...
          <div className="glass-panel hidden self-end p-6 lg:block"><p className="text-xs font-bold uppercase tracking-[0.2em] text-muted-foreground">Open today</p><p className="mt-2 font-display text-3xl">05:00—23:00</p><a href="#contact" className="mt-6 flex items-center gap-2 text-xs font-bold uppercase tracking-[0.14em] text-foreground">Get directions <ArrowRight size={14}/></a></div>
        </div>
      ...
      <section className="running-block border-y border-foreground/10 bg-surface py-5" aria-label="Club highlights"><div className="marquee-track flex min-w-max gap-12 font-display text-2xl uppercase text-muted-foreground"><span>Strength</span><b>◆</b><span>Conditioning</span><b>◆</b><span>Coaching</span><b>◆</b><span>Recovery</span><b>◆</b><span>Strength</span><b>◆</b><span>Conditioning</span><b>◆</b><span>Coaching</span><b>◆</b><span>Recovery</span></div></section>

      <section id="about" className="section-shell py-24 md:py-32">
        <div className="grid gap-12 lg:grid-cols-[0.8fr_1.2fr] lg:gap-20"><div className="reveal-block"><p className="eyebrow">01 / The standard</p><h2 className="section-title mt-5">A room that demands more.</h2></div><div className="reveal-block pt-2"><p className="max-w-2xl text-xl leading-8 text-muted-foreground md:text-2xl">Ironvault is built around one belief: your environment should raise your standard. Every station, coach, and square foot is here to help you train with intent.</p><div className="mt-12 grid grid-cols-3 gap-px bg-border"><Stat value="24/7" label="Access"/><Stat value="18K" label="Sq. ft."/><Stat value="30+" label="Coaches"/></div></div></div>
      </section>
      ...
      <section className="cta-band border-y border-foreground/10 py-16"><div className="section-shell flex flex-col justify-between gap-8 md:flex-row md:items-center"><h2 className="font-display text-5xl uppercase leading-none md:text-7xl">Ready to put in the work?</h2><Button variant="gym" size="lg" asChild><a href="#contact">Book a tour <ArrowRight/></a></Button></div></section>
